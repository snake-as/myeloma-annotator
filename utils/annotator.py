...
def annotate_gene(gene_symbol):
    ...
    # Fetch basic gene info from MyGene.info
    mygene_url = f"https://mygene.info/v3/query?q={gene_symbol}&species=human"
    r = requests.get(mygene_url).json()
    hits = r.get('hits', [])
    if hits:
        hit = hits[0]
    ...
    # Myeloma-specific markers
    myeloma_markers = [
        "CCND1", "FGFR3", "MMSET", "TP53", "BCL2", "MYC", "NRAS", "KRAS", "IKZF1", "IKZF3", "IRF4", "XBP1"
    ]
